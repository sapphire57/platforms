-- Migration to update role system with new audit-focused roles
-- This migration updates the user_role enum and migrates existing data

-- Step 1: Create new enum type with simplified role names
...
-- Step 5: Drop policies that depend on the role column first
...
-- Step 5a: Drop old column and rename new column
...
-- Step 7: Update default value for new users
ALTER TABLE public.tenant_users 
ALTER COLUMN role SET DEFAULT 'observer';

-- Step 8: Update RLS policies to work with new roles

-- Recreate policies with new role hierarchy
...
-- Step 9: Update storage policies for new roles
...
-- Step 10: Create function to check user permissions
...
-- Step 11: Update performance indexes
DROP INDEX IF EXISTS idx_tenant_users_user_role;
CREATE INDEX idx_tenant_users_user_role ON public.tenant_users(user_id, role);
CREATE INDEX idx_tenant_users_tenant_role ON public.tenant_users(tenant_id, role);

-- Step 12: Create permission levels table for granular permissions
CREATE TABLE public.permission_levels (
  id uuid DEFAULT uuid_generate_v4() PRIMARY KEY,
  name text NOT NULL UNIQUE,
  description text,
  level integer NOT NULL UNIQUE, -- Higher number = more permissions
...
-- Permission levels are readable by all authenticated users
CREATE POLICY "Permission levels are publicly readable" ON public.permission_levels
  FOR SELECT USING (auth.role() = 'authenticated');

-- Users can view permissions they have or can manage
...
-- Only managers and owners can grant permissions
...
-- Step 13: Create function to check specific permissions
...
BEGIN
  ...
  -- Get user's maximum permission level in this tenant
  ...
  -- Also check role-based permissions (roles grant implicit permissions)
...
-- Step 14: Add indexes for permission system
